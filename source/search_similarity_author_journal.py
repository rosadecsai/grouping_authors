...
def createDendogramCluster( similarity,name_out_cluster,dataAuthors,numclust=4,th=0.6,prune=False,showplot=False):
    aux=similarity.copy()
    ...
    f.close()
    #write the authors in every cluster
    for i in range(1,numclust+1):
        name_f = 'data_out/Author_Clus_All'+str(i)+'.txt'
        file_c=open(name_f,'wt')

        for pos,j in enumerate(fl):
            if (j==i):
                line = similarity.columns[pos]+";"+dataAuthors[similarity.columns[pos]]['Name']+"\n"
                file_c.write(line)
        file_c.close()        
    ...
    maximo_per_clust={1:0,2:0,3:0,4:0}
    autores_max_similares={1:(),2:(),3:(),4:()}
    nombre_autores_max_similares={1:(),2:(),3:(),4:()}
    f=open("data_out/MaxSimilares.txt",'wt')
    for i in d:
        print("Cluster ",i)
        for ath in d[i]:
            serie_aux = similarity[ath[0]].copy()
            serie_aux[ath[0]]=0
            
            idx = serie_aux.argmax()
            v = similarity.loc[ath[0],similarity.columns[idx]]

            if (v>maximo_per_clust[i]):
                maximo_per_clust[i]=v
                nombre_autores_max_similares[i]=(dataAuthors[ath[0]],dataAuthors[similarity.columns[idx]])
                autores_max_similares[i]=(ath[0],similarity.columns[idx])
        print("CLuster ",i, "Similarity ",maximo_per_clust[i], "Authors ",autores_max_similares[i][0],"-->",autores_max_similares[i][1] )
        txt="CLuster "+str(i)+"Similarity "+str(maximo_per_clust[i])+ "Authors "+str(autores_max_similares[i][0])+" "+str(nombre_autores_max_similares[i][0])+"-->"+str(autores_max_similares[i][1])+" "+str(nombre_autores_max_similares[i][1])+"\n"
        f.write(txt)
    f.close()    
    return d        
